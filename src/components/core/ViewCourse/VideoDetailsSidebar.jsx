...
export const VideoDetailsSidebar = ({setReviewModal}) => {
    ...
    useEffect(() => {
        ;(() => {
            if (!Array.isArray(courseSectionData) || !courseSectionData.length)
                return;
    ...
    <>
        <div className='flex h-[calc(100vh-3.5rem)] w-[320px] max-w-[350px] flex-col border-r-[1px]
         border-r-richblack-700 bg-richblack-800'>
            {/* Buttons & Headings */}
            <div className='mx-5 flex flex-col items-start justify-between gap-2 gap-y-4 border-b
             border-richblack-600 py-5 text-lg font-bold text-richblack-25'>
                {/* Buttons */}
                         ...
                         onClick={() => setActiveStatus(course?._id)} key={index}>
                            ...
                            <div className='flex flex-row justify-between bg-richblack-600 px-5 py-4'>
                                ...
                                <div className='flex items-center gap-3'>
                                    <span className={`${activeStatus === course?.sectionName
